...
// HX711 Strain Gauge Configuration
...
/* ========================= MODE MANAGEMENT (NEW) ========================= */
enum UIMode : uint8_t { MODE_SPEEDO=0, MODE_TORQUE=1 };
...
// Encoder MT6701 SSI pins (your schematic)
#define ENC_CS   14
#define ENC_CLK  13
#define ENC_DO   37

// TMC6300 pins (keep as is unless your six PWM pins differ)
...
static const float DETENT_SPACING_DEG    = 72.0f;
static const float HYSTERESIS_DEG        = 8.0f;
static const float KP                    = 0.30f;
...
static bool mt6701_read_deg(float& degOut){
  ...
  uint16_t ang14 = raw & 0x3FFF;
  degOut = (ang14 * 360.0f) / 16384.0f;
  return true;
}
...
#if defined(ARDUINO_ARCH_ESP32)
  int maxd = (1 << HFX_BITS) - 1;
  int ua = (int)(constrain(da, 0.0f, 1.0f) * maxd);
  int ub = (int)(constrain(db, 0.0f, 1.0f) * maxd);
  int uc = (int)(constrain(dc, 0.0f, 1.0f) * maxd);
  // unipolar PWM on highs
  pwmWrite_LEDC(0, ua); pwmWrite_LEDC(1, 0);
  pwmWrite_LEDC(2, ub); pwmWrite_LEDC(3, 0);
  pwmWrite_LEDC(4, uc); pwmWrite_LEDC(5, 0);
#else
...
static void driveBLDC(float mechDeg, float tq){
  ...
  const float A = sinf(phaseDeg * DEG_TO_RAD);
  const float B = sinf((phaseDeg - 120.0f) * DEG_TO_RAD);
  const float C = sinf((phaseDeg + 120.0f) * DEG_TO_RAD);

  float da = 0.5f + 0.5f * amp * A;
  float db = 0.5f + 0.5f * amp * B;
  float dc = 0.5f + 0.5f * amp * C;
...
static void hfx_task(void*){
  ...
  while (true){
    ...
    if (dt < 0.0008f) { vTaskDelay(pdMS_TO_TICKS(1)); continue; } // ~1 kHz
    lastMicros = now;

    float angDeg = 0;
    if (!mt6701_read_deg(angDeg)) { setPhases(0.5f,0.5f,0.5f); vTaskDelay(pdMS_TO_TICKS(2)); continue; }

    float vel = angDelta(angDeg, lastDeg) / (dt > 1e-4f ? dt : 1e-4f);
    lastDeg = angDeg;

    // nearest of 5 detents
    int   nearest = (int)floor((wrapDeg(angDeg) + DETENT_SPACING_DEG/2.0f) / DETENT_SPACING_DEG) % 5;
    ...
    if (err >  slip) { nearest = (nearest + 1) % 5; target = wrapDeg(nearest * DETENT_SPACING_DEG); err = angDelta(angDeg, target); }
    if (err < -slip) { nearest = (nearest + 4) % 5; target = wrapDeg(nearest * DETENT_SPACING_DEG); err = angDelta(angDeg, target); }

    // spring + damper + static friction
    ...
    // publish detent change -> toggle UI
...
/* ========================= TORQUE UI (separate look + NEEDLE) ========================= */
static const int   CX = 120, CY = 120;
static const int   R_IN = 70,  R_OUT = 100;
static const float TSTART = -210.0f, TEND = 30.0f;

float TORQUE_MAX_NM = 50.0f;
float torqueNm = 0.0f;       // now driven by Q <value> lines
float peakNm = 0.0f;
float oldTorqueNm = -1.0f;

static inline float mapf(float v,float a0,float a1,float b0,float b1){
...
void torque_drawBase(){
  ...
  // ticks + units (labels slightly closer)
  const int majors=10;
...
void torque_drawNeedle(float nm){
  ...
  int xb1  = cx + (int)(cosf(angle + 2.8f) * baseR);
  int yb1  = cy + (int)(sinf(angle + 2.8f) * baseR);
  int xb2  = cx + (int)(cosf(angle - 2.8f) * baseR);
  int yb2  = cy + (int)(sinf(angle - 2.8f) * baseR);
...
void torque_drawDynamic(){
  ...
  torque_drawNeedle(torqueNm);     // <<< add needle
...
void loop() {
  ...
  // --------- LINE-BASED SERIAL: S n (speed), Q n.nn (torque), or keywords ----------
  if (Serial.available() > 0) {
    String line = Serial.readStringUntil('\n');
    String up = line; up.trim(); up.toUpperCase();

    if (up == "CHANGE") { switchMode(g_mode==MODE_SPEEDO?MODE_TORQUE:MODE_SPEEDO); }
      ...
      if (nm < 0) nm = 0;
      if (nm > TORQUE_MAX_NM) nm = TORQUE_MAX_NM;
      torqueNm = nm;
      if (torqueNm > peakNm) peakNm = torqueNm;
      oldTorqueNm = -1.0f;  // force redraw
    }
    else if (up.startsWith("M ")) {
      float m = up.substring(2).toFloat();
      if (m > 1 && m <= 200){ TORQUE_MAX_NM = m; if (peakNm > TORQUE_MAX_NM) peakNm = TORQUE_MAX_NM; }
    }
    else {
      // legacy: bare integer speed or "Speed: N"
      int v = 0;
  ...
  if (millis() >= updateTime) {
    ...
    if (g_mode == MODE_SPEEDO){
      ...
      // torque shown exactly as last Q value (no HX711 influence)
  ...
  // ------ HX711 sample + smoothing (LED threshold only) ------
  ...
  Serial.print("Speed: ");
  Serial.print(speed);
  Serial.print(" km/h, Torque: ");
  Serial.print(torqueNm,1);
  Serial.print(" Nm, Strain Diff: ");
  Serial.print(strain_difference);
  Serial.print(", Thresh: ");
  Serial.println(strain_threshold);

  // LED Logic
  if (strain_difference > strain_threshold) {
    setAllLEDs(leds.Color(80, 0, 0));   // Red when pressed
  } else {
    setAllLEDs(leds.Color(0, 20, 0));   // Green when idle
  }

  // Single-char commands (+ stop/resume)
  checkSerialCommands();

  // Detent-based toggle → also prints MODE
  if (g_detent_changed){
    g_detent_changed = false;
    switchMode(g_mode==MODE_SPEEDO?MODE_TORQUE:MODE_SPEEDO);
  }
...
void drawUI() {
  tft.fillScreen(TFT_DARKBLUE);
  tft.fillCircle(120, 120, 110, TFT_DARKGRAY);
  tft.fillCircle(120, 120, 100, TFT_BLACK);
  for (int i = 0; i < 10; i++) tft.drawCircle(120, 120, 110 - i, tft.color565(40 + i*5, 40 + i*5, 40 + i*5));
  for (int i = 0; i < 5; i++)  tft.drawCircle(120, 120, 100 - i, tft.color565(20 + i*5, 20 + i*5, 20 + i*5));
  for (int i = 0; i <= 200; i += 10) {
    ...
    int x_start = cos(angle) * 85 + 120;
    int y_start = sin(angle) * 85 + 120;
    int x_end = cos(angle) * (85 + tick_length) + 120;
    int y_end = sin(angle) * (85 + tick_length) + 120;
    tft.drawLine(x_start, y_start, x_end, y_end, tick_color);
    if (i % 40 == 0) {
      int lx = cos(angle) * 70 + 120;
      int ly = sin(angle) * 70 + 120;
      tft.setTextColor(TFT_WHITE, TFT_BLACK);
...
void drawZone(int start_value, int end_value, uint16_t start_color, uint16_t end_color) {
  ...
  for (int i = 0; i < steps; i++) {
    ...
    int x0 = cos(start_angle) * 85 + 120;
    int y0 = sin(start_angle) * 85 + 120;
    int x1 = cos(end_angle) * 85 + 120;
    int y1 = sin(end_angle) * 85 + 120;
    int x2 = cos(start_angle) * 100 + 120;
    int y2 = sin(start_angle) * 100 + 120;
    int x3 = cos(end_angle) * 100 + 120;
    int y3 = sin(end_angle) * 100 + 120;
...
void checkSerialCommands() {
  if (Serial.available()) {
    ...
    switch(cmd) {
      ...
      // quick toggle key (optional)
      case 'x': switchMode(g_mode==MODE_SPEEDO?MODE_TORQUE:MODE_SPEEDO); break;
    }
